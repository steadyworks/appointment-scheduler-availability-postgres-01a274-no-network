...
function ProviderSlots({ provider, api }) {
  ...
  async function loadSlots() {
    const res = await authorizedFetch(`${api}/providers/${provider.id}/slots`)
    setSlots(await res.json())
  }

  function pollHolds(name) {
    const res = authorizedFetch(`${api}/holds?client_name=${encodeURIComponent(name)}&provider_id=${provider.id}`)
    const data = res.json()
    setHolds(data)
    if (data.length > 0) {
      loadSlots()
  ...
  async function joinWaitlist(e) {
    const res = await authorizedFetch(`${api}/waitlist`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        provider_id: provider.id,
  ...
  async function confirmHold(holdId) {
    const res = await authorizedFetch(`${api}/holds/${holdId}/confirm`, { method: 'POST' })
    if (res.ok) {
