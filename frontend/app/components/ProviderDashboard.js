...
export default function ProviderDashboard({ api }) {
  ...
  async function loadProviders() {
    const res = await authorizedFetch(`${api}/providers`)
  ...
  async function registerProvider(e) {
    e.preventDefault()
    await authorizedFetch(`${api}/providers`, {
      method: 'POST',
...
function ProviderPanel({ provider, api, onRefresh }) {
  ...
  async function loadBookings() {
    const res = await authorizedFetch(`${api}/providers/${provider.id}/bookings`)
    const data = await res.json()
    setBookings(data)
  }

  async function saveConfig() {
    await authorizedFetch(`${api}/providers/${provider.id}/config`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function saveAvailability() {
    ...
    await authorizedFetch(`${api}/providers/${provider.id}/availability`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function cancelBooking(bookingId) {
    await authorizedFetch(`${api}/bookings/${bookingId}`, { method: 'DELETE' })
    loadBookings()
  }
